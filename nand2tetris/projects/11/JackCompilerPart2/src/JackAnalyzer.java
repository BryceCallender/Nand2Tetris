...
{
    private String path = "/Users/brycecallender/desktop/Nand2Tetris/nand2tetris/projects/11";
                ...
                if(files.getName().equals(fileName))
                {
                    ...
                    for(File subDirFiles: files.listFiles())
                    {
                        if(subDirFiles.getName().contains(".jack"))
                        {
                            ...
                            String outputName = subDirFiles.getName().substring(0,subDirFiles.getName().lastIndexOf(".")) + ".xml";
                            path = path.substring(0,path.lastIndexOf("/")+1) + outputName;

                            System.out.println(path);
                            File output = new File(path);

                            outputName = subDirFiles.getName().substring(0,subDirFiles.getName().lastIndexOf(".")) + ".vm";
                            path = path.substring(0,path.lastIndexOf("/")+1) + outputName;
                            File vmFile = new File(path);

                            CompilationEngine compilationEngine = new CompilationEngine(input,output,vmFile);
                        }
